# ---------------------------------------------------------------------------------------------------------------------
# DOCKER IMAGE TERRAGRUNT CONFIGURATION — de-team-base (non-prod)
# Builds and pushes the de-team Base Image to ECR.
# Dependencies: ECR (repository URL for de-team-base)
# ---------------------------------------------------------------------------------------------------------------------
...
inputs = {
  name                = "lakehouse-at-scale-de-team-base"
  dockerfile_path     = "${dirname(find_in_parent_folders("root.hcl"))}/../dbt-dagster-project/de-team/Dockerfile.base"
  docker_context      = "${dirname(find_in_parent_folders("root.hcl"))}/../dbt-dagster-project/de-team"
  ecr_repository_url  = dependency.ecr.outputs.repository_urls["de-team-base"]
}
