...
# --- argocd/namespaces/spark-rbac.yaml ---
# The spark:spark executor SA reuses the shared de-team IRSA role (one role for driver +
# executors), so its annotation must match the de-team role ARN used in dagster/values.yaml.
$sparkRbac = "$REPO_ROOT\argocd\namespaces\spark-rbac.yaml"
if (Test-Path $sparkRbac) {
    $content = Get-Content $sparkRbac -Raw
    $content = $content -replace '(eks\.amazonaws\.com/role-arn:\s*)"[^"]*"', "`$1`"$DE_TEAM_IRSA_ROLE_ARN`""
    Set-Content $sparkRbac -Value $content -NoNewline
    Write-Host "    OK $sparkRbac"
}
...
- Spark executor SA (spark:spark): reuses de-team IRSA role
